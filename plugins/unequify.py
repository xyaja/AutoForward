...
# InlineKeyboardMarkup untuk tombol yang sudah selesai
...
# InlineKeyboardMarkup untuk tombol batal
...
async def unequify(client, message):
    ...
    if not _bot or _bot['is_bot']:
        return await message.reply("<b>You need a userbot to do this process. Please add a userbot using /settings</b>")

    target = await client.ask(user_id, text="**Forward the last message from the target chat or send the last message link.**\n/cancel - `Cancel this process`")
    ...
    confirm = await client.ask(user_id, text="**Send /yes to start the process or /no to cancel this process.**")
    ...
    try:
        ...
        await sts.edit(f"**Please make your [userbot](t.me/{_bot['username']}) an admin in the target chat with full permissions.**")
    ...
    try:
        await sts.edit(Translation.DUPLICATE_TEXT.format(total, deleted, "Processing"), reply_markup=CANCEL_BTN)
